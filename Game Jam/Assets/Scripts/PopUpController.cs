...
public class PopUpController : MonoBehaviour
{
    ...
    void Awake()
    {
        tasks = new List<string> { "bugs", "music", "art", "sleep"};
        Instance = this;
        timeLeft = 24;
    ...
    public void Update()
    {
        ...
        if (countingMethod == CountingMethod.Frames)
        {
            ...
            else
            {
                choice = "";
                counter = 0;
                if (popupUI.activeSelf == false && bugUI.activeSelf == false && musicUI.activeSelf == false && artUI.activeSelf == false && sleepUI.activeSelf == false)
                {
